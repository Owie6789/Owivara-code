 ...
 */
export function generateQRImage(qrString) {
 ...
 * @param qrString - Raw QR string from Baileys
 */
export function printQRToTerminal(qrString) {
 ...
 */
export function createQRDisplay(qrString, expiresAt = new Date(Date.now() + 5 * 60 * 1000)) {
    return {
        qr: qrString,
